...
using System.Web;
using SynapseVue.Client.Core.Services;
...
public static partial class Program
{
    ...
    public static void ConfiureMiddlewares(this WebApplication app)
    {
        ...
        app.UseCors(options => options.WithOrigins("https://0.0.0.0", "http://0.0.0.0", "0.0.0.0", "app://0.0.0.0", "http://localhost:4030", "http://localhost:6030", "http://10.0.2.2:6030")
        ...
        var blazorApp = app.MapRazorComponents<SynapseVue.Server.Components.App>()
            .AddInteractiveServerRenderMode()
            .AddInteractiveWebAssemblyRenderMode()
            .AddAdditionalAssemblies(AssemblyLoadContext.Default.Assemblies.Where(asm => asm.GetName().Name?.Contains("SynapseVue") is true).Except([Assembly.GetExecutingAssembly()]).ToArray());
