...
public static partial class Program
{
    ...
    private static void AddSwaggerGen(WebApplicationBuilder builder)
    {
        ...
        {
            options.IncludeXmlComments(Path.Combine(AppContext.BaseDirectory, "SynapseVue.Server.xml"));
            options.IncludeXmlComments(Path.Combine(AppContext.BaseDirectory, "SynapseVue.Shared.xml"));
    ...
    private static void AddHealthChecks(WebApplicationBuilder builder)
    {
        ...
        {
            setup.AddHealthCheckEndpoint("BPHealthChecks", env.IsDevelopment() ? "http://localhost:6030/healthz" : "/healthz");
        }).AddInMemoryStorage();
